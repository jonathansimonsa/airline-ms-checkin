...
namespace CheckIn.Application.UseCases.Queries.GetAllCheckIn
{
    public class GetAllCheckInHandler : IRequestHandler<GetAllCheckInQuery, List<CheckInDto>>
    {
        ...
        public GetAllCheckInHandler(ICheckInRepository checkInRepository, ILogger<GetCheckInByIdQuery> logger)
        {
        ...
        public async Task<List<CheckInDto>> Handle(GetAllCheckInQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                List<Domain.Model.CheckIn> lista = await _checkInRepository.GellAll();
